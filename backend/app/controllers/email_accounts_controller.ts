...
export default class EmailAccountsController {
    ...
    async callback({ request, response, auth }: HttpContext) {
        ...
        try {
            ...
            const oauth2Client = this.gmailOAuthService.getClient(tokens.access_token, tokens.refresh_token || null)
    ...
    async updateSettings({ params, request, response, auth, bouncer }: HttpContext) {
        ...
        const { autoDeleteEmails, autoScanEnabled } = request.only(['autoDeleteEmails', 'autoScanEnabled'])
